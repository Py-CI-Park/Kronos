...
def _candidate_scores(path: Path) -> None:
...
def test_authority_audit_verifies_bound_price_and_pit_receipts(tmp_path: Path) -> None:
    ...
    # Then: the research-data authority clears without opening TEST or promotion.
    assert receipt.status == "VERIFIED_RESEARCH_DATA_AUTHORITY"
    assert receipt.d0_price_basis.state == "VERIFIED"
    assert receipt.d0_price_basis.price_basis == "split_adjusted"
    assert receipt.d1_universe.state == "VERIFIED"
