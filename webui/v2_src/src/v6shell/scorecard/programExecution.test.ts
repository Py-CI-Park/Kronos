...
test('execution banner preserves the research boundary and delivery lineage', () => {
  ...
  assert.match(PROGRAM_EXECUTION.deliveryLane, /codex\/D5/);
  assert.equal(PROGRAM_EXECUTION.baseRelease, 'fork-v1.14.0-kronos-rl-d4-algorithm-objective');
  assert.equal(PROGRAM_EXECUTION.stage, 'D5_FULL_TRAIN_COST_NOT_CONFIRMED');
  assert.equal(PROGRAM_EXECUTION.authority, 'REVIEWED_SNAPSHOT');
  assert.equal(PROGRAM_EXECUTION.reviewedRun, 'type2-d5-primary-20260729-001');
  assert.match(PROGRAM_EXECUTION.reviewedEvidenceManifest, /^[0-9a-f]{64}$/);
  assert.match(PROGRAM_EXECUTION.nextAction, /D5R/i);
});
