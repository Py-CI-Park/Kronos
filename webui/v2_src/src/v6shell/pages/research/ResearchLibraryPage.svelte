<script lang="ts">
  ...
  const visibleBlocked = $derived(page?.items.filter((row) => row.status.includes('BLOCK') || row.status.includes('MISMATCH') || row.status.includes('TEST_FEATURES_CONSUMED')).length ?? 0);
  const visibleCandidates = $derived(page?.items.filter((row) => row.status === 'VALIDATION_CANDIDATE').length ?? 0);
  const kpis = $derived([
    ...
    { label: 'BLOCKED / INVALID', value: String(visibleBlocked), detail: '권위·재현·TEST 오염 차단', tone: 'danger' as const },
    { label: 'CANDIDATE', value: String(visibleCandidates), detail: '다음 검증 후보 · 승격 아님', tone: 'warning' as const },
  ]);
  const statusChartItems = $derived.by(() => {
    ...
    return [...counts.entries()].map(([label, value]) => ({
      label,
      value,
      tone: runStatusTone(label) === 'danger' ? 'danger' as const : runStatusTone(label) === 'warning' ? 'warning' as const : 'accent' as const,
    }));
  });
      ...
      <div class="rows" role="list" aria-label="연구 실행 목록">
        {#each page.items as row}
          ...
          <article role="listitem">
            ...
            <dl><div><dt>판정</dt><dd data-status={row.status} data-tone={runStatusTone(row.status)}>{runStatusLabel(row.status)}</dd></div><div><dt>알고리즘</dt><dd>{row.algorithm}</dd></div><div><dt>데이터셋</dt><dd>{row.dataset_id}</dd></div><div><dt>증거</dt><dd>{row.source_file}</dd></div><div class="health"><dt>결과 근거</dt><dd><span><i style={`width:${health.percent}%`}></i></span>{health.observed}/{health.total}<small>{health.missing.length ? `없음: ${health.missing.join('·')}` : '핵심 metadata 관측'}</small></dd></div></dl>
