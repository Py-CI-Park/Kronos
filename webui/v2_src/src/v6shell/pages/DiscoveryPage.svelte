...
<section class="discovery-page" aria-labelledby="discovery-title">
  ...
  {#if loading}
    <section class="state" aria-live="polite">최신 Primary 증거를 읽는 중입니다.</section>
  {:else if error}
    <section class="state error" role="alert"><strong>Artifact 오류</strong><p>{error}</p></section>
  {:else if !evidence}
  ...
  {:else}
    ...
    <section class="arms" aria-labelledby="arms-title">
      <div class="section-title"><p>CONTROL MATRIX</p><h2 id="arms-title">D0 arm × seed 상세</h2></div>
      <div class="arm-grid">{#each evidence.arms as arm}<article><div class="arm-head"><span>{armLabel(arm.id)} <b>SEED {arm.seed}</b></span><strong class={outcomeClass(arm.oracleRewardRatio)}>{arm.oracleRewardRatio.toFixed(3)}×</strong></div><div class="meter" aria-label={`${arm.id} seed ${arm.seed} oracle reward ratio`}><i style:width={ratioWidth(arm.oracleRewardRatio)}></i></div><dl><div><dt>Oracle reward</dt><dd>{arm.oracleRewardRatio.toFixed(3)}×</dd></div><div><dt>Basket accuracy</dt><dd>{pct(arm.exactBasketAccuracy)}</dd></div><div><dt>Dominant action</dt><dd class:danger={arm.dominantActionRate >= .95}>{pct(arm.dominantActionRate)}</dd></div><div><dt>Train steps</dt><dd>{arm.trainingTimesteps.toLocaleString()}</dd></div></dl><p class="validity">invalid {arm.invalidActionCount} · block {arm.blockCount} · no-fill {arm.noFillCount}</p>{#if arm.shuffledReward}<span class="control">NEGATIVE CONTROL</span>{/if}</article>{/each}</div>
    </section>
