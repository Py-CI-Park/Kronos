...
<div class="evidence" data-evidence-page>
  ...
  <div class="grid">
    <ResearchPanel title="판정 분포" description="NO-GO와 손상 증거를 숨기지 않습니다."><div class="status-list">{#each Object.entries(summary?.catalog.by_status ?? {}) as [status, count]}<div><span>{status}</span><strong>{count}</strong></div>{:else}<p>판정 집계가 없습니다.</p>{/each}</div></ResearchPanel>
...
<style>
  .evidence{display:flex;flex-direction:column;gap:16px;min-width:0}.grid{display:grid;grid-template-columns:minmax(0,.8fr) minmax(0,1.2fr);gap:16px}.status-list{display:grid;gap:7px}.status-list div{display:flex;justify-content:space-between;gap:12px;border-bottom:1px solid var(--border);padding:7px 0}.status-list span{color:var(--fg);font:.7rem var(--font-mono)}.status-list strong{color:var(--fg-strong)}ol{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:0;padding:0;list-style:none}li{display:flex;align-items:center;gap:9px;border:1px solid var(--border);border-radius:8px;padding:9px;background:var(--surface-sunken)}li b{display:grid;place-items:center;flex:0 0 auto;width:24px;height:24px;border-radius:50%;background:var(--accent);color:var(--on-accent);font-size:.65rem}li span{color:var(--fg);font-size:.68rem}.error{border:1px solid var(--danger);border-radius:8px;padding:12px;color:var(--danger)}
  @media(max-width:820px){.grid{grid-template-columns:1fr}}@media(max-width:560px){ol{grid-template-columns:1fr}}
</style>
