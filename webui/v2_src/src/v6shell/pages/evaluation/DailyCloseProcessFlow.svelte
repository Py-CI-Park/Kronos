  ...
  <ol>
    {#each steps as step, index}
      <li class:active={active === index} class:complete={index < active} aria-current={active === index ? 'step' : undefined}>
        <button class="step-button" type="button" aria-label={`${index + 1}단계 ${step.title} 상세 보기`} aria-pressed={active === index} onclick={() => selectStep(index)}>
          <span>{String(index + 1).padStart(2, '0')}</span><div><code>{step.code}</code><strong>{step.title}</strong><small>{step.detail}</small></div>
        </button>
      </li>
    {/each}
  </ol>
  <aside aria-live="polite" aria-label="선택한 단계 상세">
...
<style>
  .flow{min-width:0;border:1px solid var(--border);border-radius:14px;background:var(--surface-raised);overflow:hidden}.flow>header{display:flex;align-items:end;justify-content:space-between;gap:16px;padding:16px 18px;border-bottom:1px solid var(--border)}header p{margin:0;color:var(--accent);font:900 .58rem var(--font-mono);letter-spacing:.1em}header h2{margin:4px 0;color:var(--fg-strong);font-size:1.1rem}header span{color:var(--muted);font-size:.7rem}nav{display:flex;gap:7px}button{border:1px solid var(--border-strong);border-radius:7px;padding:7px 9px;background:var(--surface-sunken);color:var(--fg);font-weight:800;cursor:pointer}ol{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:0;margin:0;padding:0;list-style:none}li{position:relative;min-width:0;border-right:1px solid var(--border);border-bottom:1px solid var(--border);opacity:.62;transition:background .25s ease,opacity .25s ease}li:nth-child(3n){border-right:0}li:nth-last-child(-n+3){border-bottom:0}li.active{opacity:1;background:color-mix(in srgb,var(--accent) 12%,var(--surface-raised))}li.complete{opacity:.82}.step-button{display:grid;grid-template-columns:auto 1fr;gap:10px;width:100%;height:100%;min-height:92px;border:0;border-radius:0;padding:14px;background:transparent;text-align:left}.step-button:hover,.step-button:focus-visible{background:color-mix(in srgb,var(--accent) 8%,transparent)}.step-button>span{display:grid;place-items:center;width:28px;height:28px;border:1px solid var(--border-strong);border-radius:50%;color:var(--muted);font:800 .62rem var(--font-mono)}li.active .step-button>span{border-color:var(--accent);background:var(--accent);color:var(--on-accent);animation:pulse 1s ease-in-out infinite}.step-button div{min-width:0;display:flex;flex-direction:column;gap:3px}code{color:var(--accent);font-size:.56rem}strong{color:var(--fg-strong);font-size:.77rem}small{color:var(--muted);font-size:.64rem;line-height:1.45}.flow>aside{display:grid;grid-template-columns:minmax(220px,.8fr) minmax(0,1.5fr);gap:18px;padding:16px 18px;background:color-mix(in srgb,var(--accent) 7%,var(--surface-sunken));border-top:1px solid var(--border)}aside>div>span{color:var(--accent);font:900 .56rem var(--font-mono)}aside p{margin:5px 0 0;color:var(--muted);font-size:.68rem}aside dl{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin:0}aside dl div{border-left:2px solid var(--border-strong);padding-left:9px}aside dt{color:var(--dim);font:.56rem var(--font-mono)}aside dd{margin:3px 0 0;color:var(--fg);font-size:.67rem;line-height:1.45}@keyframes pulse{50%{transform:scale(1.08)}}
  @media(max-width:900px){ol{grid-template-columns:1fr 1fr}li:nth-child(3n){border-right:1px solid var(--border)}li:nth-child(2n){border-right:0}li:nth-last-child(-n+3){border-bottom:1px solid var(--border)}li:last-child{border-bottom:0}.flow>header{align-items:start;flex-direction:column}.flow>aside{grid-template-columns:1fr}.flow>aside dl{grid-template-columns:1fr 1fr 1fr}}
  @media(max-width:560px){ol{grid-template-columns:1fr}li,li:nth-child(3n){border-right:0}.flow>header nav{width:100%}.flow>header button{flex:1}.flow>aside dl{grid-template-columns:1fr}}
