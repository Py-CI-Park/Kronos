<script lang="ts">
  ...
  async function refresh(): Promise<void> {
    ...
    loading = true;
    const [leftResult, rightResult] = await Promise.all([loadTelemetry(leftId), loadTelemetry(rightId)]);
    if (leftResult.ok === false) error = leftResult.message;
    else if (rightResult.ok === false) error = rightResult.message;
    else {
...
<div class="evaluation" data-evaluation-workspace>
  ...
  <ResearchPanel title="비교 실행 선택" description="왼쪽 실행과 같은 lane의 실행만 오른쪽 후보로 허용합니다.">
    <div class="selectors"><label>기준 실행<select bind:value={leftId} onchange={selectLeft}>{#each runs as run}<option value={run.run_id}>{run.lane} · {run.status} · {run.name}</option>{/each}</select></label><label>비교 실행<select bind:value={rightId} onchange={() => void refresh()}>{#each sameLaneRuns as run}<option value={run.run_id} disabled={run.run_id === leftId}>{run.status} · {run.name}</option>{/each}</select></label><button type="button" onclick={() => void refresh()} disabled={loading}>비교 새로고침</button></div>
  </ResearchPanel>
