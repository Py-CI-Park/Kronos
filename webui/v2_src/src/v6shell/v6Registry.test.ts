...
test('V6 registry keeps workflow steps and implementation status honest', () => {
  ...
  assert.deepEqual(V6_PAGES.filter((page) => ['overview', 'data', 'experiment', 'training'].includes(page.id)).map((page) => page.status), ['BUILT', 'BUILT', 'BUILT', 'BUILT']);
  assert.ok(V6_PAGES.filter((page) => !['overview', 'data', 'experiment', 'training'].includes(page.id)).every((page) => page.status === 'NOT_BUILT'));
});
