...
// Resolves a canonical dashboard CSS custom property at option-build time so
// evidence charts follow the active light/dark theme instead of fixed hex.
// The fallback preserves the previously hardcoded value for non-browser contexts.
...
function colorByValue(value: unknown): string {
  return Number(value) >= 0 ? readVar('--success', '#16a34a') : readVar('--danger', '#dc2626');
}

export function leaderboardChartOption(rows: readonly RlTableRow[]): Option {
  const sample = rows.slice(0, 12);
  if (!sample.length) return {};
  const axisColor = readVar('--dim', '#64748b');
  return {
    ...
    xAxis: { type: 'category', data: sample.map((row) => text(row, 'model', text(row, 'policy'))), axisLabel: { color: axisColor, rotate: 20 } },
    yAxis: { type: 'value', name: 'net/MDD %', axisLabel: { formatter: '{value}%', color: axisColor } },
    series: [
      { name: 'avg episode net', type: 'bar', data: sample.map((row) => rowNumber(row, 'avg_episode_net_return_pct')), itemStyle: { color: readVar('--accent', '#0f766e'), borderRadius: [5, 5, 0, 0] } },
      { name: 'MDD', type: 'bar', data: sample.map((row) => rowNumber(row, 'max_drawdown_pct')), itemStyle: { color: readVar('--danger', '#ef4444'), borderRadius: [5, 5, 0, 0] } },
    ],
  };
}
...
export function costGateChartOption(rows: readonly RlTableRow[]): Option {
  ...
  return {
    ...
    xAxis: { type: 'category', data: rows.map((row) => text(row, 'policy')), axisLabel: { color: axisColor, rotate: 20 } },
    yAxis: { type: 'value', name: 'net %', axisLabel: { formatter: '{value}%', color: axisColor } },
...
export function equityChartOption(rows: readonly RlTableRow[]): Option {
  ...
  return {
    ...
    xAxis: { type: 'category', data: rows.map((row, idx) => text(row, 'timestamp', text(row, 'step', String(idx + 1))).slice(0, 19)), axisLabel: { color: axisColor } },
    yAxis: { type: 'value', name: 'net evidence %', axisLabel: { formatter: '{value}%', color: axisColor } },
    series: [{ name: 'Time equity curve', type: 'line', smooth: 0.2, symbol: 'none', data: rows.map((row) => useCandidateNetReturn ? rowNumber(row, 'net_return_pct') : (rowNumber(row, 'equity', 1) - 1) * 100), lineStyle: { color: readVar('--accent', '#0f766e'), width: 2.4 } }],
  };
}
...
export function actionPnlChartOption(actions: readonly RlTableRow[], episodes: readonly RlTableRow[], selectedName: string): Option {
  ...
  return {
    ...
    xAxis: { type: 'category', data: rows.map((point) => String(point.idx + 1)), axisLabel: { color: axisColor } },
    yAxis: { type: 'value', name: 'cumulative reward %', axisLabel: { formatter: '{value}%', color: axisColor } },
    series: [
      { name: 'model cumulative reward', type: 'line', symbol: 'none', data: rows.map((point) => point.cumulativeRewardPct), lineStyle: { color: readVar('--accent', '#7c3aed'), width: 2.4 }, markLine: { silent: true, symbol: 'none', label: { show: false }, data: rows.filter((point) => point.boundary).map((point) => ({ xAxis: point.idx })) } },
      { name: 'ts_imb baseline', type: 'line', symbol: 'none', data: rows.map((point) => point.baseline), lineStyle: { color: readVar('--muted', '#f59e0b'), type: 'dashed', width: 2 } },
    ],
  };
}
...
export function tradeChartOption(rows: readonly RlTableRow[]): Option {
  ...
  return {
    ...
    xAxis: { type: 'category', data: sample.map((row, idx) => text(row, 'symbol', text(row, 'policy', String(idx + 1)))), axisLabel: { color: axisColor, rotate: sample.length > 18 ? 40 : 0 } },
    yAxis: { type: 'value', name: 'trade net %', axisLabel: { formatter: '{value}%', color: axisColor } },
