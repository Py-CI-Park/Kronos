...
from .daily_market_errors import DailyMarketArtifactError
...
def resolve_trusted_artifact(
    ...
    *,
    ...
    if raw_root.is_symlink() or raw_path.is_symlink():
        raise DailyMarketArtifactError(f"{label}:SYMLINK_NOT_ALLOWED")
    ...
    try:
        ...
        raise DailyMarketArtifactError(f"{label}:OUTSIDE_TRUSTED_ARTIFACT_ROOT") from exc
    if not resolved_path.is_file():
        raise FileNotFoundError(resolved_path)
    size = resolved_path.stat().st_size
    if size <= 0 or size > max_bytes:
        raise DailyMarketArtifactError(f"{label}:ARTIFACT_SIZE_OUT_OF_BOUNDS")
