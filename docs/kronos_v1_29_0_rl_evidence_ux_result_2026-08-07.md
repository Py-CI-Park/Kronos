...
|---|---:|---:|---|
| 연구 catalog | 98개 수준의 상위/특수 항목 혼합 | 229개 실행·그룹 증거 단위 | 직접 index가 있는 그룹도 하위 실행을 숨기지 않음 |
...
| 정적 자산 | 이전 해시 | `index-BxOWQAgc.js` | 단위 안전·모바일·검토 후속 수정까지 새 UI build 반영 |
...
1. **연구 라이브러리**: 전체 결과 수가 229개이고 run id가 `그룹/실행` 형태인지 확인한다.
...
| 검토 지적 | 최종 조치 | 증거 |
|---|---|---|
| JSON telemetry 경계의 `typing.cast` | Pydantic `TypeAdapter(dict[str, JsonValue])`로 교체 | 변경 Python Basedpyright `0 errors, 0 warnings` |
| event 파일 symlink 추적 가능성 | API와 reader 양쪽에서 symlink 거부 | symlink guard 회귀 테스트 |
| `action_recorded=false` 행동 표시 | 분포·타임라인·시간대·최근 결정에서 모두 제외 | 행동 모델 회귀 테스트 |
| direct index가 있는 그룹의 child run 누락 | 그룹 증거와 직접 child run을 함께 catalog에 포함 | catalog 229, child 보존 테스트 |
| 누락 reward를 0처럼 연결 | 미관측 reward point를 chart series에서 제외 | reward gap 회귀 테스트 |
| 통합 route 충돌 오해 | research blueprint 뒤 telemetry blueprint를 등록한 통합 테스트 추가 | `limit=20`은 HTTP 200, 계약 밖 `limit=5`는 의도된 HTTP 400 |
