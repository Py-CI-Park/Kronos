...
**진입·첫 30초 마이크로구조로 "이 트레이드가 결국 SL로 끝날지"는 *적당히 예측 가능*하다(full N=5,173, AUC 0.58–0.61, symbol-disjoint 0.58–0.59, 4개 모델 전부 사전등록 바 통과). → 사전등록대로 GO: 실험 ①(skip-gate)·④(상태조건 청산)의 전제(조건 걸 신호 존재)가 *기각되지 않았다*.**

**중요한 해석 한 줄: 이건 *방향성 알파*가 아니라 *리스크(변동성) 예측*이다.** "−1% 먼저 칠지(SL)"는 하방 변동성 문제이고, microstructure가 *방향*은 못 맞춰도(P1b NO-GO·shuffle 무알파) *변동성/리스크*는 맞춘다는 건 문헌과 데이터레이어 평가(1초봉=리스크 레이어)와 정확히 정합. **돈이 되는지는 별개** — 그건 ①이 직접 검정한다.

> 이번 게이트는 이 전체 RL/ML 조사에서 **처음 나온 비-음성 결과**다. 과대해석 금물: AUC 0.60은 "동전보다 조금 나음"이고, SL 예측이 곧 수익은 아니다.
...
|---|---|---|---|
| **entry** | 진입봉~첫 5초 | ① skip-gate("진입 시 스킵?") | 없음(전 5,173) |
| **path30** | 첫 30초 | ④ 상태청산("30초 보유 중, SL로 끝날까?") | 30초 시점 *미청산*만(4,218) |
...
|---|---|---:|---|---:|---|
| entry (n=5,173) | logit | **0.601** | [0.572, 0.630] | 0.594 | **PREDICTABLE** |
| entry | gbm | 0.578 | [0.548, 0.608] | 0.576 | PREDICTABLE |
| path30 (n=4,218) | logit | **0.605** | [0.575, 0.635] | 0.594 | PREDICTABLE |
| path30 | gbm | 0.578 | [0.549, 0.607] | 0.575 | PREDICTABLE |

- **4개 모델 전부** point AUC≥0.55·CI 하한>0.5·symbol-disjoint≥0.53 통과. logit이 GBM보다 약간 우수(선형으로 충분, 과적합 신호 아님). symbol-disjoint(미관측 종목)서도 0.58–0.59 유지 → per-ticker 암기 아닌 **일반화되는 리스크 신호**.
- entry·path30 거의 동일(0.60 vs 0.61) → SL 예측력의 대부분이 *진입 시점*에 이미 존재(첫 30초가 크게 더 주지 않음). ①(진입 스킵)에 유리한 방향.
- **FINAL: GO.**
...
2. **④ 상태청산: 살아남음(약).** 30초 경로도 SL을 0.61로 예측 → 조건변수가 비어있지 않음. 단 entry≈path30라 "보유 중 추가정보"가 작고, 갭상승 평균회귀라는 구조적 반대(데이터레이어 §4)는 그대로 → ① 다음 후순위.
3. **그러나 "예측 가능 ≠ 수익".** 핵심 함정(데이터레이어 평가가 P1을 태운 **드리프트 트랩**): forward 드리프트가 대체로 양수라, SL로 끝나는 트레이드조차 *비용차감 net이 음수가 아닐* 수 있다. AUC 0.60 SL예측은 "SL 많은 슬라이스를 식별"할 뿐, 그 슬라이스를 스킵해 **돈이 남는지**는 ①이 직접 검정해야 함. SL 트레이드도 일부는 −1% 손절 전 +α 먹고 시간청산됐을 수 있음(라벨은 *최종* 이유라 net 부호와 1:1 아님).
...
1. AUC 0.60은 **modest**(완벽 1.0·동전 0.5 사이 하단). 통계적으론 견고(CI·symbol-disjoint·4모델 일치)하나 실거래 분리력은 작다 — "강한 예측"이 아니라 "0이 아닌 예측".
...
3. ts_imb 트리거 모집단 한정·L2 없음. positive/negative control이 게이트 검출력 보증.
4. 사전등록 0.55 바는 임의성 최소화 선택. 결과는 그 바를 명확히 통과(0.578–0.605)하므로 바 민감도 낮음.
5. RULE not RL. 본 실험은 수익 산출이 아니라 "조건 걸 신호 존재" 측정. 모든 후속 양수치는 in-sample/triggered-subset/라이브 없음.
...
```
산출: `.omx/artifacts/sl_predictor/summary.json`. 핵심(2026-05-31): N=5,173, SL률 59.4%, open_at_30s=4,218, AUC 0.578–0.605(symbol-disjoint 0.575–0.594), 4모델 PREDICTABLE, FINAL GO.
...
| **③ SL예측 선행 분류기** | ✅ **완료 — PREDICTABLE(AUC 0.58–0.61, 강건), FINAL GO** |
| **① skip-gate** | ⬜ **다음 — ③가 전제 통과, 빌드 정당화(드리프트 트랩 가드 사전등록 필수)** |
| ④ 상태조건 청산 | ⬜ ① 이후(약한 사전확률, 평균회귀 반대) |
